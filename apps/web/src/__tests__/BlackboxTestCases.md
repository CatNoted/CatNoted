...
| **TC-BLK-013** | Collapsible Views Section Toggle | Sidebar visible. | Click on 'VIEWS' header or chevron icon. | 'VIEWS' section smoothly expands or collapses, hiding or showing navigation options (Doc Mode, Canvas, Graph, Settings). | Navigation options are completely hidden when collapsed, without affecting sidebar width. |
| **TC-BLK-014** | Workspace Switcher Navigation | Sidebar visible. | Click workspace button, then click a different workspace in the popover. | Workspace popover closes, active workspace text updates, and active checkmark reflects selection. | Only current workspace has the active checkmark styling in the dropdown. |
