...
export const GenericShape: React.FC<GenericShapeProps> = ({
  ...
  if (element.type === 'note') {
    shapeClass += 'bg-yellow-100/90 dark:bg-yellow-900/90 rounded text-foreground ';
  ...
  return (
    ...
    >
      <div className={`${shapeClass} w-full h-full relative overflow-hidden group justify-center transition-shadow ${isSelected ? 'shadow-md shadow-primary/10' : ''}`}>
        {element.type === 'frame' ? (
           <span className="absolute top-2 left-3 text-[10px] font-mono font-medium text-muted-foreground bg-background/80 px-1.5 py-0.5 rounded shadow-sm z-10">Frame</span>
        ) : null}
        ...
        {element.type !== 'frame' && (
          <textarea
            ...
            className={`w-full h-full bg-transparent resize-none ${alignClass} outline-none focus:ring-0 p-2 placeholder-muted-foreground/50 flex flex-col justify-center text-sm`}
            placeholder="Type..."
            style={{
