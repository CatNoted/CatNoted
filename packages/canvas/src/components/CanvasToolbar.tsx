...
export const CanvasToolbar: React.FC<CanvasToolbarProps> = ({
  ...
  const getButtonClass = (id: string) => {
    ...
    if (activeFeedback === id) {
      return `${base} bg-primary text-white scale-90 dark:bg-primary dark:text-primary-foreground`;
    }
    return `${base} hover:bg-secondary text-muted-foreground hover:text-primary dark:hover:text-primary`;
  };

  return (
    <div className="absolute bottom-6 left-1/2 -translate-x-1/2 z-40 bg-background/90 backdrop-blur-md border border-border/60 rounded-2xl p-2 flex items-center gap-1.5 shadow-xl shadow-sm dark:shadow-black/20 hover:shadow-2xl hover:-translate-y-1 transition-all duration-300">
      <button
      ...
      {onToggleHelp && (
        <>
          ...
          <button
            ...
            className={`${getButtonClass('help')} ${isHelpActive ? 'text-primary dark:text-primary bg-primary/10 dark:bg-primary/10' : ''}`}
