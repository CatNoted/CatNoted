...
export const CanvasProperties: React.FC<CanvasPropertiesProps> = ({ selectedElements, onUpdateElement, elements }) => {
  ...
  if (element.type === 'connector') {
    ...
    return (
      <div className="absolute top-6 right-6 z-40 bg-card/90 backdrop-blur-md border border-border/60 rounded-2xl p-4 flex flex-col gap-5 shadow-xl shadow-border/5 w-64 animate-in fade-in slide-in-from-right-4 duration-200">
        <div className="text-[10px] font-mono font-semibold text-muted-foreground uppercase tracking-widest border-b border-border pb-2 flex items-center justify-between">
          ...
          <span className="bg-muted text-primary px-1.5 py-0.5 rounded text-[9px]">connector</span>
        </div>
  ...
  return (
    <div className="absolute top-6 right-6 z-40 bg-card/90 backdrop-blur-md border border-border/60 rounded-2xl p-4 flex flex-col gap-5 shadow-xl shadow-border/5 w-64 animate-in fade-in slide-in-from-right-4 duration-200">
      <div className="text-[10px] font-mono font-semibold text-muted-foreground uppercase tracking-widest border-b border-border pb-2 flex items-center justify-between">
        <span>Properties</span>
        <span className="bg-muted text-primary px-1.5 py-0.5 rounded text-[9px]">{element.type}</span>
      </div>
        ...
        <div className="flex flex-wrap gap-1.5">
          {colors.map(color => (
            <button
              ...
              className={`w-6 h-6 rounded-md border border-border ${color.replace('bg-', 'bg-').replace('-100', '-100 dark:bg-opacity-20')} hover:scale-110 transition-transform ${element.color === color ? 'ring-2 ring-primary ring-offset-1 ring-offset-background' : ''}`}
              title={color}
      ...
      <div className="flex flex-col gap-2.5">
        <div className="flex items-center justify-between">
          ...
          <span className="text-xs font-mono text-muted-foreground/80 bg-muted px-1.5 rounded">
      ...
      <div className="flex flex-col gap-2">
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className="text-foreground/90 text-[10px] font-semibold rounded-md border border-border px-2 py-1 hover:bg-muted"
            type="button"
          >
            Front
          </button>
      ...
      <div className="flex flex-col gap-2.5 pt-3 border-t border-border/80">
        ...
        <div className="flex items-center gap-1">
          <button
            ...
            className="text-[10px] font-semibold rounded-md border border-border px-2 py-1 text-foreground/90 hover:bg-muted"
            type="button"
          >
            Up
          </button>
